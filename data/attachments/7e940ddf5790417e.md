...
Locator: locator('a, button, li').filter({ hasText: /^delete$/i }).filter({ visible: true }).first()
Expected: visible
Timeout: 3000ms
...
Call log:
  ...
  - waiting for locator('a, button, li').filter({ hasText: /^delete$/i }).filter({ visible: true }).first()
