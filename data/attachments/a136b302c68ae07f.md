...
- Name: ../.features-gen/features/12-보관함/Wait-until-Free.feature.spec.js >> Wait until Free >> [TPS-184] Wait until Free 메뉴 클릭 + 상단 < 버튼 클릭
- Location: .features-gen/features/12-보관함/Wait-until-Free.feature.spec.js:6:7
...
Locator: locator('a.item-title[href*="WAIT_UNTIL_FREE"]')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('a.item-title[href*="WAIT_UNTIL_FREE"]')
...
> 223 |   await expect(page.locator('a.item-title[href*="WAIT_UNTIL_FREE"]')).toBeVisible();
  ...
  312 | 
  313 | // 유저 홈으로 이동된다. — 프로필-more.steps.ts에서 처리
  314 | 
  315 | Then(/^(뷰어로 이동된다\.|설정된 랜딩페이지로).+$/, async ({ page }) => {
  316 |   await expect(page.locator('body')).toBeVisible();
  317 | });
  318 | 
  319 | Then(/^\[Get\]버튼 > \[Read\]로 변경된다\.$/, async ({ page }) => {
  320 |   const readBtn = page.locator('.inbox-gift-item__btn-read, button.js-inbox-gift-read').first();
  321 |   const isRead = await readBtn.isVisible().catch(() => false);
  322 |   if (isRead) { await expect(readBtn).toBeVisible(); return; }
  323 |   const readByText = page.locator('.inbox-gift-item').filter({ has: page.getByRole('button', { name: /^read$/i }) }).first();
```