...
Feature: Spotlight (섹션 서브탭)
  ...
  @TC-0071
  Scenario: [TPS-064] 미로그인 / 미인증 상태 > 미로그인 / 미인증 아이디 로그인 상태 + mature 클릭
  ...
  @TC-0072
  Scenario: [TPS-065] 미로그인 / 미인증 상태 > M 뱃지 노출되는 작품 클릭 + Submit 버튼 클릭
    Given 미로그인 / 미인증 상태
  ...
  @TC-0075
  Scenario: [TPS-068] 빅배너 영역에서 8초 이상 대기 + 빅배너 우로 스와이프
  ...
  @TC-0077
  Scenario: [TPS-070] Top 섹션 > 카드배너 노출 확인 + 카드배너 클릭
  ...
  @TC-0078
  Scenario: [TPS-071] 라인배너 섹션 노출 확인 + 라인배너 클릭
    When 라인배너 섹션 노출 확인
    And 라인배너 클릭
    And 상단네비바 [<] 또는 단말 백버튼 클릭
  ...
  @TC-0079
  Scenario: [TPS-072] 섹션메뉴 노출 확인 + 더보기> 영역 클릭
    When 섹션메뉴 노출 확인
    And 더보기[>] 영역 클릭
