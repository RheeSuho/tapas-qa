...
Feature: 하단 툴바
  ...
  @TC-0136
  Scenario: [TPS-088] PCW only - 전체화면
    Given PCW only
    When [전체화면] 버튼 클릭
    And [전체화면] 버튼 재클릭
  ...
  @TC-0137
  Scenario: [TPS-089] 소설 뷰어 옵션(AA) - 폰트 크기
    When [AA] 버튼 클릭
    And 폰트 크기 [+] 버튼 클릭
    And 폰트 크기 [-] 버튼 클릭
  ...
  @TC-0138
  Scenario: [TPS-090] 소설 뷰어 옵션(AA) - 행 간격
    When [AA] 버튼 클릭
    And 행 간격 [+] 버튼 클릭
    And 행 간격 [-] 버튼 클릭
  ...
  @TC-0139
  Scenario: [TPS-091] 소설 뷰어 옵션(AA) - 화면 모드
    When [AA] 버튼 클릭
    And 뷰어 화면 모드 클릭
    And 팝업 이외 영역 클릭
  ...
  @TC-0142
  Scenario: [TPS-094] 첫 번째 작가 서포트 활성화
