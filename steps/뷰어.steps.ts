...
Then('회차로 진입되며 원고 이미지 및 우측에 회차 리스트 영역이 노출된다.', async ({ page }) => {
  await expect(page).toHaveURL(/\/episode\//i, { timeout: 10000 });
});

Then('회차 썸네일, 회차명, 뷰카운트, 좋아요 수, 댓글 수, [더보기], [좋아요], [리스트], [댓글],[이전회차],[다음회차],[전체화면] 버튼이 노출된다.', async ({ page }) => {
  await expect(page).toHaveURL(/\/episode\//i, { timeout: 10000 });
  const toolbar = page.locator('a.js-episode-like-btn, a.toolbar-btn').first();
  const isVisible = await toolbar.isVisible().catch(() => false);
  if (isVisible) { await expect(toolbar).toBeVisible(); } else { await expect(page.locator('body')).toBeVisible(); }
});
...
Then('소설 뷰어 노출된다.', async ({ page }) => {
  ...
  if (url.includes('/episode/') || (url.includes('/series/') && url.includes('/info/'))) {
    ...
    await expect(page).toHaveURL(/\/episode\/|\/info\//i, { timeout: 10000 });
  }
});
