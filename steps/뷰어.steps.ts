...
Then('뷰어 원고 이미지와 리스트 버튼이 노출된다.', async ({ page }) => {
  await expect(page.locator('img.content__img').first()).toBeVisible({ timeout: 8000 });
  await assertToolbarBtn(page, 'a.toolbar-btn.js-list-btn');
});
...
Then('[Add a comment] 버튼이 노출된다.', async ({ page }) => {
  await expect(page.getByRole('button', { name: /add a comment/i }).first()).toBeVisible({ timeout: 5000 });
});
...
Then('Style 팝업이 유지된다.', async ({ page }) => {
  await expect(page.locator('.js-edit-menu').first()).toBeVisible({ timeout: 5000 });
});
